...
async function startServer() {
  try {
    ...
    // Important: Skip /api/* paths to prevent catching API requests
    const distPath = path.join(__dirname, '..', 'dist');
    if (fs.existsSync(distPath)) {
      // Middleware to skip API routes before static file serving
      app.use((req, res, next) => {
        // Skip static file serving for /api/* paths
        if (req.path.startsWith('/api/')) {
          return next();
        }
        // Otherwise, try to serve static files
      ...
      console.log('[server] serving static files from:', distPath, '(skipping /api/* paths)');
    } else {
