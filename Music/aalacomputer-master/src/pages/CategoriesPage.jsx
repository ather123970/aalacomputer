...
const CategoriesPage = () => {
  ...
  const loadCategoriesWithProducts = async () => {
    ...
    try {
      ...
      // Auto-categorize products and count
      const counts = {};
      
      products.forEach(product => {
        let productCategory = product.category;
        
        // If no category or invalid, auto-detect
        if (!productCategory || productCategory.trim() === '') {
          const detected = autoDetectCategory(product);
          productCategory = detected ? detected.name : 'Uncategorized';
        }

        // Find which predefined category this product belongs to
        let matchedCategoryName = productCategory;
        
        // Try to match against our predefined categories
        ...
        const matchedCategory = PC_HARDWARE_CATEGORIES.find(cat => {
          ...
          return (
            ...
            normalizedProductCategory.includes(normalizedSlug) ||
            normalizedSlug.includes(normalizedProductCategory)
          );
        });
      ...
      if (counts['Uncategorized']) {
        enrichedCategories.push({
          ...
          productCount: counts['Uncategorized'],
          brands: []
        });
      }

      setCategories(enrichedCategories.filter(cat => cat.productCount > 0));
