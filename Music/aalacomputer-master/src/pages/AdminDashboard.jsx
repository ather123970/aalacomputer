...
const ProductModal = ({ product, onClose, onSave, categories = [] }) => {
  ...
  useEffect(() => {
    const imgValue = product?.img || product?.imageUrl || '';
    setFormData(prev => ({
      ...
      img: imgValue,  // Use same value for both fields
      imageUrl: imgValue,  // Use same value for both fields
      ...
      ...product,
      // Override img and imageUrl to ensure they're synced
      img: imgValue,
      imageUrl: imgValue
    }));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Ensure both img and imageUrl use the same value
      const imageValue = formData.img || formData.imageUrl || '';
      ...
      const payload = {
        ...
        stock,
        sold,
        img: imageValue,  // Sync both fields
        imageUrl: imageValue,  // Sync both fields
      ...
      <motion.div initial={{ scale: 0.98 }} animate={{ scale: 1 }} exit={{ scale: 0.98 }} className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 border">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-sm text-gray-600">Image URL (Local or External)</label>
              <input 
                type="text" 
                value={formData.img} 
                onChange={e => setFormData({
                  ...formData, 
                  img: e.target.value,
                  imageUrl: e.target.value  // Sync both fields
                })} 
                className="w-full px-3 py-2 border rounded" 
                placeholder="/images/product.jpg or https://example.com/image.jpg" 
              />
              <p className="text-xs text-gray-500 mt-1">
                Local: /images/file.jpg | External: https://...
              </p>
            </div>
          </div>
