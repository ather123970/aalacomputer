...
/**
 * Generic Category Search Results Component
 * Works for ANY product category (GPUs, CPUs, RAM, etc.)
 */
export default function CategorySearchResults({ results, query, category, isLoading = false }) {
  ...
  if (isLoading) {
    return (
      <div className="bg-card rounded-xl border border-gray-700 p-8 text-center">
        ...
        <p className="text-muted">Searching for {category}...</p>
      </div>
    );
  }
  ...
  if (!displayResults || displayResults.length === 0) {
    return (
      <div className="bg-card rounded-xl border border-gray-700 p-8 text-center">
        ...
        <p className="text-lg font-semibold text-primary mb-2">No {category} Found</p>
        <p className="text-muted text-sm mb-4">
          Try searching with different terms or browse our {category.toLowerCase()} collection.
        </p>
        <button
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl p-6 text-white">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-2xl font-bold">{category} Search Results</h2>
        </div>
        <p className="text-blue-100">
          Found {displayResults.length} {category.toLowerCase()}{displayResults.length !== 1 ? 's' : ''} from aalacomputer.com
        </p>
      ...
      <div className="bg-card rounded-xl border border-gray-700 p-4 text-center">
        <p className="text-sm text-muted">
          All products are from <span className="font-semibold text-blue-400">aalacomputer.com</span>
        </p>
      </div>
    </div>
  );
...
/**
 * Individual Product Result Card
 */
function ProductResultCard({ product, navigate, rank }) {
  const handleClick = () => {
