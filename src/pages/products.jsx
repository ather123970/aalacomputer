...
const Products = () => {
  ...
  useEffect(() => {
    ...
    const aliasMap = {
      ...
      deals: ["deals", "deal", "offer", "discount"],
      prebuilds: ["prebuilds", "prebuild", "pre-build", "pre build"],
      laptops: ["laptops", "laptop", "notebook"],
      motherboard: ["motherboard", "mobo", "mainboard", "mb"],
      cpu: ["cpu", "processor", "intel", "amd"],
      gpu: ["gpu", "graphics card", "graphic card", "graphics", "vga", "video card"],
      ram: ["ram", "memory", "ddr", "ddr4", "ddr5"],
      storage: ["storage", "ssd", "hdd", "nvme", "m2", "m.2", "hard drive"],
      psu: ["psu", "power supply", "smps"],
      case: ["case", "casing", "pc case", "chassis"],
      cooling: ["cooling", "cooler", "fan", "liquid cooling", "aio"],
      ...
      "motherboard accessories": ["motherboard accessories", "cables", "adapters"],
      "laptop accessories": ["laptop accessories", "charger", "adapter", "laptop charger"]
    };
    const canon = (s) => {
      const v = norm(s);
      const direct = norm(s);
    ...
    if (searchQuery.trim()) {
      ...
      base = base.filter((p) => {
        ...
        return nameStr.includes(query) || specStr.includes(query) || catStr.includes(query);
      });
    }
